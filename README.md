...
2. Add **Public Hostnames** to the tunnel (service type **HTTP** — TLS terminates at Cloudflare's edge; the tunnel talks plain HTTP to the containers):
   ...
   If you add the `api.*` hostname, also set `FEEDBIN_API_HOST=api.feedbin.example.com` in `.env` — without it, Rails' `HostAuthorization` middleware rejects requests with that Host header (`Blocked hosts: api.feedbin.example.com` in `web` logs) even though the tunnel routes them correctly. If `FEEDBIN_HOST` has more than 2 labels (true whenever it's a subdomain of your own domain, e.g. `feedbin.example.com` rather than a bare `feedbin.com`), also set `TLD_LENGTH=2` — otherwise Feedbin's `/v2/*` API routes (subdomain-gated) 404 with `ActionController::RoutingError: No route matches [GET] "/v2/..."` even after HostAuthorization lets the request through. See the design note below for why.
...
- **`TLD_LENGTH` for the `api.*` hostname's routes to actually match.** Feedbin's `/v2/*` API routes are wrapped in `constraints subdomain: "api"` in its `routes.rb`. Rails computes a request's `subdomain` by treating the last `TLD_LENGTH + 1` labels of the Host header as the "domain" and everything before that as the subdomain (`config.action_dispatch.tld_length`, default `1`). Feedbin assumes its own canonical 2-label root domain (`feedbin.com`), where `TLD_LENGTH=1` makes `api.feedbin.com` resolve to subdomain `"api"`. This stack's `FEEDBIN_HOST` is usually itself a subdomain of your real domain (e.g. `feedbin.example.com`, 3 labels), so with the default `TLD_LENGTH=1`, `api.feedbin.example.com` resolves to subdomain `"api.feedbin"` — the constraint never matches `"api"`, and every API request 404s with `ActionController::RoutingError` even though HostAuthorization already let it through. Set `TLD_LENGTH=2` to shift the base domain calculation out by one label so `"api"` alone is left as the subdomain.
